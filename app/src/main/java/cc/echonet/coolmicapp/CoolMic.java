...
public class CoolMic {
    ...
    public String getStreamStatsURL() {
        ...
        System.err.println(String.format("http://%s:%s@%s/admin/stats.xml?mount=/%s", this.getUsername(), this.getPassword(), this.getServerName(), this.getMountpoint()));
    ...
    public boolean isCMTSConnection() {
        if(
            this.getServerName().contains("coolmic.net") || this.getServerName().contains("cc.echonet.cc") ||
